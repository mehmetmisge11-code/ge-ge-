...
internal suspend fun loadAllTargets(context: Context): List<Target> {
    ...
    val cache = PoiStore(context).load()
    return buildTargets(places, cache, 600, 300)
}
...
class TrackerService : Service() {
    ...
    private fun applyTier(newTier: Int, force: Boolean = false) {
        ...
        val slow: Long = if (mode == TrackMode.SAVER) 3L else 1L
        val priority: Int = if (newTier == TIER_HUNT) {
            Priority.PRIORITY_HIGH_ACCURACY
        } else {
            Priority.PRIORITY_BALANCED_POWER_ACCURACY
        }
        val intervalMs: Long = when (newTier) {
            TIER_HUNT -> 1_000L * slow
            TIER_NEAR -> 20_000L * slow
            TIER_IDLE -> 300_000L
            else -> 90_000L * slow
        }
    ...
    private fun maybeRefresh(loc: Location, now: Long) {
        ...
        scope.launch {
            runCatching {
                ...
                val newArea = Area.name(loc.latitude, loc.longitude)
