    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                context.applicationContext.startTrackerIfPossible()
            } finally {
                pending.finish()
            }
        }
...
internal suspend fun loadTargets(context: Context): List<Target> =
...
class ProximityService : Service() {
    ...
    /**
     * DIKKAT - "GecGec yanit vermiyor" hatasinin sebebi buydu.
     *
     * Konum olcumleri saniyede bir geliyor ve her olcumde bildirim kurmak,
     * zil sesi hazirlamak, uygulama acmak gerekiyor. Bunlar ana is parcaciginda
     * yapilirsa Android 5 saniye sonra "uygulama dondu" diyor.
     *
     * Bu yuzden olcumler kendi arka plan hattinda karsilaniyor.
     * BURAYI TEKRAR getMainLooper() YAPMA.
     */
    ...
    private fun startTracking() {
        ...
        val t = HandlerThread("gecgec-konum").also { it.start() }
        worker = t
        client.requestLocationUpdates(req, callback!!, t.looper)
    }
