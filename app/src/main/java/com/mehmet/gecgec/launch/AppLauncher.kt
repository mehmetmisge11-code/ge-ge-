...
object AppLauncher {
    ...
    /** Arka planda calisir - "Dene"ye basinca ekran donmasin. */
    fun test(context: Context, place: Place) {
        val app = context.applicationContext
        Thread { fire(app, place.copy(cooldownMinutes = 0), "test", place.id) }.start()
    }
    ...
    fun fire(context: Context, place: Place, why: String, key: String = place.id) {
        ...
        // Ogrenme servisi bunu "kullanici kendi acti" sanmasin
        markLaunched(context, place.targetPackage)
    ...
    /**
     * Zil sesi ARKA PLANDA hazirlanir.
     * getRingtone() ses dosyasini diskten acip cozuyor - ana is parcaciginda
     * saniyeler surebiliyor ve "uygulama yanit vermiyor" hatasi veriyordu.
     */
    ...
    // ---- Ogrenme servisi icin ----

    private fun markLaunched(context: Context, pkg: String) {
        if (pkg.isBlank()) return
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .edit().putLong("launched_$pkg", System.currentTimeMillis()).apply()
    }

    /** Son 3 dakikada bu uygulamayi GecGec mi acti? */
    fun launchedRecently(context: Context, pkg: String): Boolean {
        val t = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .getLong("launched_$pkg", 0L)
        return System.currentTimeMillis() - t < 3 * 60_000L
    }
}
