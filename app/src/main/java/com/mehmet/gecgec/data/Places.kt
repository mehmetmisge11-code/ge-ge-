...
object Area {
    ...
    fun name(context: Context, lat: Double, lng: Double): String {
        // ~110 metrelik cozunurluk - ayni sokakta tekrar sorma
        val key = "%.3f,%.3f".format(Locale.US, lat, lng)
        if (key == cachedKey) return cachedName

        val found = runCatching {
            if (!Geocoder.isPresent()) return@runCatching ""
            val g = Geocoder(context, Locale("tr", "TR"))
            @Suppress("DEPRECATION")
            val hit = g.getFromLocation(lat, lng, 1)?.firstOrNull() ?: return@runCatching ""
            val mahalle = hit.subLocality?.takeIf { it.isNotBlank() }
            val ilce = hit.locality?.takeIf { it.isNotBlank() }
                ?: hit.subAdminArea?.takeIf { it.isNotBlank() }
            listOfNotNull(mahalle, ilce).distinct().joinToString(", ")
        }.getOrDefault("")
