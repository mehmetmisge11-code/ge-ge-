...
fun buildTargets(places: List<Place>, cache: PoiCache): List<Target> {
    ...
    for (p in places.filter { it.enabled && it.isReady }) {
        when (p.kind) {
            ...
            PlaceKind.BRAND ->
                brandPois(p.id, cache).take(MAX_PER_BRAND).forEach { poi ->
                    out += Target(
    ...
    return out.distinctBy { it.fenceId }.take(MAX_FENCES)
}
...
class PoiStore(private val context: Context) {
    ...
    fun isStale(cache: PoiCache, lat: Double, lng: Double): Boolean {
        ...
        return distanceMeters(lat, lng, cache.centerLat, cache.centerLng) > 1200.0
    }
     ...
     */
    suspend fun addManual(placeId: String, lat: Double, lng: Double, label: String): Boolean {
        val c = load()
        val near = { p: Poi -> distanceMeters(p.lat, p.lng, lat, lng) < 150.0 }
        // Zaten biliyorsak (haritadan ya da elden) tekrar ekleme
        if (c.manual[placeId].orEmpty().any(near)) return false
        if (c.byPlace[placeId].orEmpty().any(near)) return false
    ...
    suspend fun blockPoi(placeId: String, poi: Poi) {
        val c = load()
...
object MapSearch {
    ...
    suspend fun sweepCity(
        ...
        val all = mutableListOf<Poi>()
        val stepLat = 0.10          // ~11 km
        val stepLng = 0.125         // ~11 km (Turkiye enleminde)
        val grid = -2..2
        val total = 25
        var done = 0
        ...
        for (i in grid) {
            for (j in grid) {
                val cy = lat + i * stepLat
                val cx = lng + j * stepLng
                val part = if (key.isNotBlank()) {
                    val r = get(
                    ...
                    nearby(context, text, cy, cx)
                }
                all += part.filter { matchesBrand(it.label, text) }
                done++
